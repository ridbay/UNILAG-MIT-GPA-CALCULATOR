...
interface WelcomeScreenProps {
  ...
  onLogin: (officialName?: string) => void;
}
...
export function WelcomeScreen({
  ...
  const studentMatch = STUDENTS.find((s) => s.matric === matricNumber);

  const isNameMatch = () => {
    if (!studentMatch || !name.trim()) return false;
    const inputParts = name.toLowerCase().trim().split(/\s+/);
    const actualNameParts = studentMatch.name.toLowerCase().split(/\s+/);

    return inputParts.every((part) =>
      actualNameParts.some((actualPart) => actualPart.includes(part)),
    );
  };

  const isStudentFound = !!studentMatch;
  const isNameValid = isNameMatch();
  const isValid = isValidMatric && isStudentFound && isNameValid;
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-slate-900 via-emerald-950 to-slate-900">
      ...
      <div
        className="orb w-64 h-64 bg-blue-500 top-1/2 left-1/4 animate-float"
          ...
          <div className="space-y-6 relative z-10">
            <div className="relative">
              <label
                ...
                className="block text-xs font-bold text-emerald-400/70 uppercase tracking-wider mb-3"
              >
                Full Name
              </label>
              <div className="relative group/input">
                <div
                  className={`absolute -inset-0.5 bg-gradient-to-r from-emerald-500 to-blue-500 rounded-xl blur opacity-0 group-hover/input:opacity-30 transition duration-500 ${name.length > 0 ? "opacity-30" : ""}`}
                />
                <input
                  type="text"
                  id="student-name"
                  className={`relative w-full px-6 py-4 bg-slate-900/50 border border-white/10 rounded-xl text-white placeholder-white/20 focus:border-emerald-500 focus:bg-slate-900/80 transition-all duration-300 outline-none text-xl font-medium tracking-wider backdrop-blur-sm`}
              ...
              {name.trim().length > 0 &&
                ...
                !isNameValid && (
                  <p className="absolute -bottom-6 left-0 text-sm text-red-400 flex items-center gap-1.5 animate-slide-up">
                    ...
                    Name does not match this matric number
                  </p>
                )}
            </div>
            ...
            <div className="relative">
              ...
              <div className="relative group/input">
                <div
                  className={`absolute -inset-0.5 bg-gradient-to-r from-emerald-500 to-blue-500 rounded-xl blur opacity-0 group-hover/input:opacity-30 transition duration-500 ${hasInput ? "opacity-30" : ""}`}
                />
                <input
                  type="text"
                  id="matric-number"
